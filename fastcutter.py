 ...
 ***************************************************************************/
"""
from qgis.PyQt.QtCore import QSettings, QTranslator, QCoreApplicationF
...
class CutterPlugin:
    ...
    def run(self):
        ...
        QgsMessageLog.logMessage(
            "FastCutter: Avvio plugin...", "FastCutter", level=Qgis.Info)

        # 1. Controlli iniziali
        layer = self.iface.activeLayer()
        if not layer:
            self.iface.messageBar().pushMessage(
                "Errore", "Nessun layer selezionato!", level=Qgis.Critical)
            return

        if layer.type() != layer.VectorLayer:
            self.iface.messageBar().pushMessage(
                "Errore", "Il layer attivo non è vettoriale!", level=Qgis.Critical)
            return

        selection = layer.selectedFeatures()
        if not selection:
            self.iface.messageBar().pushMessage("Errore",
                                                "Seleziona almeno una feature da usare come coltello!", level=Qgis.Critical)
            return

        # --- DEBUG: Info selezione ---
        num_cutters = len(selection)
        QgsMessageLog.logMessage(
            f"FastCutter: Selezionate {num_cutters} feature come coltello.", "FastCutter", level=Qgis.Info)
        ...
        try:
            ...
            if num_cutters > 1:
                for i in range(1, num_cutters):
                    feat = selection[i]
                    combined_cutter_geom = combined_cutter_geom.combine(
                        feat.geometry())
                    cutter_ids.append(feat.id())

            QgsMessageLog.logMessage(
                "FastCutter: Geometria coltello combinata creata con successo.", "FastCutter", level=Qgis.Info)

        except Exception as e:
            self.iface.messageBar().pushMessage(
                "Errore", f"Errore nella creazione del coltello: {e}", level=Qgis.Critical)
            QgsMessageLog.logMessage(
                f"FastCutter ERROR: {e}", "FastCutter", level=Qgis.Critical)
            return
        ...
        if not layer.isEditable():
            layer.startEditing()
            QgsMessageLog.logMessage(
                "FastCutter: Editing avviato automaticamente.", "FastCutter", level=Qgis.Info)
        ...
        for feature in layer.getFeatures(request):
            ...
            if feature.geometry().intersects(combined_cutter_geom):
                try:
                    ...
                    QgsMessageLog.logMessage(
                        f"FastCutter ERROR su feature {feature.id()}: {e}", "FastCutter", level=Qgis.Warning)

        QgsMessageLog.logMessage(
            "FastCutter: Analizzate {processed_count} feature, Tagliate {count}.", "FastCutter", level=Qgis.Info)
        ...
        if count > 0:
            ...
            QMessageBox.information(
                self.iface.mainWindow(), "FastCutter - Risultato", msg)
        else:
